...
public class SessionManager extends CompositeService {
    ...
    public synchronized void closeSession(SessionHandle sessionHandle) throws HiveSQLException {
        ...
        LOG.info("会话关闭  Session closed, " + sessionHandle + ", current sessions:" + getOpenSessionCount());
        try {
            session.close();
        } finally {
            ...
            if (!(hiveServer2 == null) && (hiveConf.getBoolVar(ConfVars.HIVE_SERVER2_SUPPORT_DYNAMIC_SERVICE_DISCOVERY)) && (hiveServer2.isDeregisteredWithZooKeeper())) {
                ...
                if (getOpenSessionCount() == 0) {
                    ...
                    Thread shutdownThread = new Thread() {
                        @Override
                        public void run() {
                            LOG.info("会话关闭 hiveServer2 服务停止阿 ");
                            hiveServer2.stop();
                        }
                    };
    ...
    public HiveSession getSession(SessionHandle sessionHandle) throws HiveSQLException {
        LOG.info("HiveSession getSession  获取 ===== {}", sessionHandle);
    ...
    public static void setProxyUserName(String userName) {
        LOG.info("setting proxy user name based on query param to: " + userName);
    ...
    private void executeSessionHooks(HiveSession session) throws Exception {
        List<HiveSessionHook> sessionHooks = HookUtils.readHooksFromConf(hiveConf, HiveConf.ConfVars.HIVE_SERVER2_SESSION_HOOK);
