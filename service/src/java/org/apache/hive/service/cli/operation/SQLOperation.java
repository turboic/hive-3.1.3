...
public class SQLOperation extends ExecuteStatementOperation {
  ...
  private void runQuery() throws HiveSQLException {
    try {
      ...
      LOG.info("SQL 运行查询的状态是 === {}",opState.name());
  ...
  private final class BackgroundWork implements Runnable {
    ...
    private BackgroundWork(UserGroupInformation currentUGI,
      ...
      LOG.info("BackgroundWork构造方法创建  李小姐  === {}",this);
    }
    ...
    public void run() {
      PrivilegedExceptionAction<Object> doAsAction = new PrivilegedExceptionAction<Object>() {
        ...
        public Object run() throws HiveSQLException {
          LOG.info("BackgroundWork的线程方法run运行逻辑,设置hive  李小姐  === {}",parentHive);
          Hive.set(parentHive);
          ...
          try {
            if (asyncPrepare) {
              LOG.info("BackgroundWork的线程方法r prepare(queryState)  === {}",asyncPrepare);
              prepare(queryState);
            }
            runQuery();
          } catch (HiveSQLException e) {
