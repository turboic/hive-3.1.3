...
public class RemoteHiveSparkClient implements HiveSparkClient {
  ...
  RemoteHiveSparkClient(HiveConf hiveConf, Map<String, String> conf, String sessionId) throws Exception {
    ...
    LOG.info("创建远程客户端 ==== {}====配置信息==={}  ======== sparkConf ==== {}",sessionId,conf,sparkConf);
    createRemoteClient();
  }
  ...
  private void createRemoteClient() throws Exception {
    ...
    if (HiveConf.getBoolVar(hiveConf, ConfVars.HIVE_PREWARM_ENABLED) &&
      ...
      if (minExecutors <= 0) {
        LOG.info("创建远程客户端 minExecutors 小于 0  ===== {}",minExecutors);
        return;
      }
      ...
      LOG.info("创建远程客户端 maxPrewarmTime  ===== {}",maxPrewarmTime);

      long ts = System.currentTimeMillis();

      LOG.info("创建远程客户端   ===当前时间戳 == {}",ts);
      do {
        try {
          ...
          LOG.error("Timed out getting executor count.", e);
        }
        if (curExecutors >= minExecutors) {
          LOG.error("Finished prewarming Spark executors. The current number of executors is " + curExecutors);
          return;
        }
  ...
  public SparkJobRef execute(final DriverContext driverContext, final SparkWork sparkWork)
    ...
    if (SparkClientUtilities.isYarnMaster(hiveConf.get("spark.master")) &&
      ...
      close();
      LOG.info("SparkJobRef execute ======= close finished ");
  ...
  private void refreshLocalResources(SparkWork sparkWork, HiveConf conf) throws IOException {
    ...
    LOG.info("SparkJobRef refreshLocalResources ======= sparkWork ===== {}, HiveConf ====== {} ",sparkWork,conf);

    // add hive-exec jar
    addJars((new JobConf(this.getClass())).getJar());

    LOG.info("SparkJobRef refreshLocalResources ======= this.getClass() ===== {}",this.getClass().getName());


    // add aux jars

    LOG.info("SparkJobRef refreshLocalResources ======= conf.getAuxJars() ===== {}",conf.getAuxJars());
    ...
    LOG.info("SparkJobRef refreshLocalResources ======= JobConf ===== {}  ======= {}",
            addedJars,HiveConf.ConfVars.HIVEADDEDJARS);
    ...
    for (BaseWork work : sparkWork.getAllWork()) {
      ...
      LOG.info("SparkJobRef refreshLocalResources ======= BaseWork ===== {}  jobConf ======= {}",
              work,jobConf);
    }
    addJars(jobConf.get(MR_JAR_PROPERTY));

    LOG.info("SparkJobRef refreshLocalResources ======= addJars(jobConf.get(MR_JAR_PROPERTY)) ===== {}",
            jobConf.get(MR_JAR_PROPERTY));


    // remove the location of container tokens
    conf.unset(MR_CREDENTIALS_LOCATION_PROPERTY);
    LOG.info("SparkJobRef refreshLocalResources ======= conf.unset(MR_CREDENTIALS_LOCATION_PROPERTY) ===== {}",
            MR_CREDENTIALS_LOCATION_PROPERTY);
    ...
    LOG.info("SparkJobRef refreshLocalResources ===哈哈哈 -》 {}==== addResources(addedFiles) ===== {}",HiveConf.ConfVars.HIVEADDEDFILES,addedFiles);
    ...
    LOG.info("SparkJobRef refreshLocalResources = ===哈哈哈 -》 {}====== addResources(addedArchives) ===== {}",HiveConf.ConfVars.HIVEADDEDARCHIVES,addedArchives);
