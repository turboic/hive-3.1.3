...
public class RemoteDriver {
    ...
    private RemoteDriver(String[] args) throws Exception {
        ...
        this.clientRpc = Rpc.createClient(mapConf, egroup, serverAddress, serverPort, clientId, secret, protocol).get();
        this.running = true;
    ...
    private void addShutdownHook() {
        LOG.info("添加线程钩子 ======== {}", "addShutdownHook");
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            if (running) {
                LOG.info("Received signal SIGTERM, attempting safe shutdown of Remote Spark Context");
                protocol.sendErrorMessage("Remote Spark Context was shutdown because it received a SIGTERM " + "signal. Most likely due to a kill request via YARN.");
                shutdown(null);
            }
        }));
    }
    ...
    private void submit(JobWrapper<?> job) {
        synchronized (jcLock) {
            if (jc != null) {
                LOG.info("Job任务开始提交Wrapper ======== {}", job);
                job.submit();
            } else {
                LOG.info("SparkContext not yet up, queueing job request.进一岁有一岁的欢喜\n" + "天坛马面裙值得！");
                jobQueue.add(job);
            }
        }
    }
    ...
    private class JobWrapper<T extends Serializable> implements Callable<Void> {
        ...
        public Void call() throws Exception {
          ...
          LOG.info("希望我们的人生中都有音乐和爱 call ====== {}",req.id);

            try {
                jc.setMonitorCb(new MonitorCallback() {
                    @Override
                    public void call(JavaFutureAction<?> future, SparkCounters sparkCounters, Set<Integer> cachedRDDIds) {
                      LOG.info("监控方法进行回调阿 ===== {} ======{} ======={}",future,sparkCounters,cachedRDDIds);
        ...
        void submit() {
            ...
            LOG.info("Job任务开始提交Wrapper ========   submit  {}", this.future);
        }
        ...
        void releaseCache() {
            if (cachedRDDIds != null) {
                for (Integer cachedRDDId : cachedRDDIds) {
                  LOG.info("Job任务开始releaseCache ========   cachedRDDId  {}", cachedRDDId);
        ...
        private void monitorJob(JavaFutureAction<?> job, SparkCounters sparkCounters, Set<Integer> cachedRDDIds) {
            ...
            LOG.info("Job任务开始提交Wrapper ========   monitorJob ，req.id ==== {},sparkJobId ====== {}", req.id, sparkJobId);
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            LOG.info("RemoteDriver ==== RemoteDriver ====== {}", args);
            rd.run();
            LOG.info("希望我们的人生中都有音乐和爱 ====== 运行啦");
        } catch (Exception e) {
