...
class SparkClientImpl implements SparkClient {
    ...
    SparkClientImpl(RpcServer rpcServer, Map<String, String> conf, HiveConf hiveConf, String sessionid) throws IOException {
        ...
        LOG.info("SparkClientImpl 实现类配置 ===== {}", conf);
        this.conf = conf;
        ...
        try {
            ...
            LOG.info("注册客户端，会话id ===== {}，密钥 ===== {}，协议 ====== {}", sessionid, secret, protocol);
            this.driverRpc = rpcServer.registerClient(sessionid, secret, protocol).get();
        } catch (Throwable e) {
          LOG.info("注册客户端，出现异常啦，兔子",e);
          ...
          String errorMsg = null;
            ...
            try {
                ...
                // Give up.
                LOG.error("线程被打断   Interrupted before driver thread was finished.", ie);
            }
    ...
    public Future<?> addJar(URI uri) {
        LOG.info("jar URI 信息是PATH  ======== {}", uri.toString());
    ...
    private Thread startDriver(final RpcServer rpcServer, final String clientId, final String secret) throws IOException {
        ...
        LOG.info("壬二酸祛痘凝胶  Running client driver with argv: {}", cmd);
        ...
        if (password != null) {
            LOG.info("hive的密码不为空，开始设置密码喽 password: {}", password);
            pb.environment().put(Constants.HADOOP_CREDENTIAL_PASSWORD_ENVVAR, password);
        }
        if (isTesting != null) {
            LOG.info("hive的isTesting不为空，开始设置isTesting isTesting: {}", isTesting);
    ...
    private class ClientProtocol extends BaseProtocol {
        ...
        <T extends Serializable> Future<T> run(Job<T> job) {
            java.security.ProtectionDomain protectionDomain = BaseProtocol.SyncJobRequest.class.getProtectionDomain();
            CodeSource codeSource = protectionDomain.getCodeSource();
            LOG.info("SyncJobRequest Class from path ============= {}", codeSource.getLocation());
            @SuppressWarnings("unchecked") final io.netty.util.concurrent.Future<T> rpc = (io.netty.util.concurrent.Future<T>) driverRpc.call(new BaseProtocol.SyncJobRequest(job), Serializable.class);
            return rpc;
        }
